...
loadingManager.onLoad = function () {
    ...
    // Si le moteur n'est pas actif/visible, on stoppe tout pour économiser les performances
    ...
    if (engineModel) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            // Force l'affichage en block au cas où le CSS les cache au démarrage
            ...
            if (h.line && h.textBlock) {
                ...
                // OPTIMISATION : On applique la structure et les positions une seule fois
                if (!h.initialized) {
                    ...
                    // On ne valide l'initialisation que si les coordonnées calculées sont valides et l'élément affiché
                    if (x > 0 && y > 0) {
                        h.initialized = true;
                    }
                }
    ...
    // Redimensionnement optimisé avec un léger debounce implicite (évite les spams)
    ...
    // --- CODE DU FORMULAIRE DE CONTACT OPTIMISÉ ---
    ...
    contactForm.addEventListener("submit", async function (e) {
        ...
        const response = await secureFetch("auth_process.php", {
        method: "POST",
        body: formData,
        });
